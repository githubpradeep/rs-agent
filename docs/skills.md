...
- If the skill is about a large-context workflow (long docs, big diffs, whole-repo sweeps),
  mention the RLM `repl` tool (`llm_query`/`agent_query`) explicitly — that's the mechanism for
...
in `~/.rs-agent/skills/`. There's no package registry yet (see `TODO.md` section H); until then,
share skills the same way you'd share a gist or a dotfiles repo.
